...
class linerModel:
    ...
    def J(self,theta, X, Y):
        ...
        '''
        m = len(X)
        return np.sum(np.dot((self.h(theta, X) - Y).T, (self.h(theta, X) - Y)) / (2 * m))
    ...
    def bgd(self
        ...
        while count <= maxloop:  # 当学习次数小于最大学习次数限制时，循环可以运行
            ...
            theta1 = 参数1 - 学习率 * (1 / 数据总长度的值) * sum(当前参数0，1代入假设函数临时得到的直线 - 真实结果的值) * 真实参数  <----(这里的真是参数与前面的计算结果是点乘)
            '''
            theta0 = theta[0, 0] - alpha / m * (self.h(theta, X) - Y).sum()
            theta1 = theta[1, 0] - alpha / m * (
                np.dot(X[:, 1][:, np.newaxis].T, (self.h(theta, X) - Y))  # 这里的是矩阵对向量的叉乘
            ).sum()  # 重点注意一下
            # 同步更新
            theta[0, 0] = theta0
            theta[1, 0] = theta1
            ...
            # 更新当前代价变量的值cost
            cost = self.J(theta, X, Y)
            costs.append(cost)

            # 如果收敛，则不再迭代
            if cost < epsilon:  # 如果迭代次数设置为无穷大，则强行满足收敛标志，退出迭代
                converged = True
    ...
    def liner_plot(self
        ...
        resault = self.linerRegression(X, Y, alpha, maxloop, epsilon)
        res = resault[0]
        plt.scatter(X, Y ,c='g')
...
if __name__ == '__main__':
    ...
    print(X.shape)
    print(Y.shape)
    print(linerModel().liner_plot(X,Y,alpha=0.000001,maxloop=1000,epsilon=0.1))
