# -*- encoding: utf-8 -*-
'''
@File    :   Pass_of_class_A_K_means.py
@Time    :   2020/09/07 13:46:10
@Author  :   DataMagician 
@Version :   1.0
...
def log10_sum_of_errors(new_k , old_k):
    '''
    误差对数和
    '''
    return np.log10(old_k-new_k).sum()
...
def K_means1(dataset, K_point, restart=0):
    '''标准计算循环完成K-means,误差log10和版本'''
    print('restart', restart)
    ...
    if log10_sum_of_errors(np.array(K_point), np.array(new_K_point)) == 0:  # 误差平方和
        print('Ok',log10_sum_of_errors(np.array(K_point), np.array(new_K_point)))
        return np.array(new_K_point), classified_of_data
    else:  # 如果不满足分类条件，则递归继续
        restart += 1
        return K_means1(dataset, new_K_point, restart)


#暂时不可用，需要对log2为低的误差和做对冲修改 ，也许可以在误差和结果后**2
def K_means_run1(class_number, all_datas, drawing=False):
    '''
    误差log2和版本
    :param class_number:
    :param all_datas:
    ...
    '''
    k_of_constant = K_point(class_number)
    result = K_means1(all_datas, k_of_constant, 0)
...
if __name__ == '__main__':
    ...
    plt.show()
    plt.close()
    # print(K_means_run(6, data, True))
    #K_means_run1(6, data, True)
    #K_means_run(5,data,True)
    K_Medoide_run(5,data,True)
    #K_Medoide_run(5, data, True)
    print(random_point(5))
